...
<!-- prettier-ignore -->
:::caution[`--public --no-password` is blocked]
...
- **Experimental Ghostty-Web engine** — selected from the in-app settings and loaded on demand from packaged JavaScript and WebAssembly assets. It uses the same authenticated WebSocket connection as the default xterm.js engine.
