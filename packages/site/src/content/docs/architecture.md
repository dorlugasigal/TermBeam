...
- **Inline terminal images** — the xterm.js `ImageAddon` decodes Kitty graphics (including Unicode placeholders), Sixel, and iTerm image protocol output. A 32 MB encoded-data compatibility cache preserves Kitty placement-only deletion semantics until the upstream beta implements them. Per-terminal pixel, decoded-image storage, protocol payload, retained-data, and Sixel palette limits bound browser memory use.
