...
- **Inline terminal images** — the xterm.js `ImageAddon` decodes Kitty graphics (including Unicode placeholders), Sixel, and iTerm image protocol output. A narrow compatibility patch preserves Kitty image data for placement-only deletion until the upstream beta distinguishes it from explicit image-data deletion. Per-terminal pixel, decoded-image storage, protocol payload, and Sixel palette limits bound browser memory use.
